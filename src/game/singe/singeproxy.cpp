...
/* Reload a sprite from its original file path into g_sprites without touching
 * g_firstload.  Used during savestate restore to reconstruct the sprite table.
...
static void sep_sprite_reload_path(const std::string &path, int frames)
{
    ...
    if (!path.empty()) {
        ...
        if (frames > 0) {
            raw = g_rom_zip ? sep_surface_zip(path) : IMG_Load(path.c_str());
        } else {
#if SDL_IMAGE_VERSION_AT_LEAST(2, 6, 0)
            IMG_Animation *anim = g_rom_zip ? sep_animation_zip(path) : IMG_LoadAnimation(path.c_str());
            if (anim) {
                if (anim->count < 2) {
                    ...
                    if (g_colorkey) {
                        for (int x = 0; x < anim->count; x++)
                            SDL_SetColorKey(anim->frames[x], SDL_TRUE, 0x0);
                    }
        ...
        if (raw) {
            ...
            if (convert != raw) SDL_FreeSurface(raw);
            if (convert) {
                SDL_SetSurfaceRLE(convert, SDL_TRUE);
                if (g_colorkey) SDL_SetColorKey(convert, SDL_TRUE, 0x0);
...
size_t sep_serialize_lua(uint8_t *buf, size_t max)
{
    ...
    uint32_t spcount = (uint32_t)g_sprites.size();
    lsw32(&p, spcount);
    for (uint32_t i = 0; i < spcount && p < end-8; i++) {
        int32_t fr = g_sprites[i].frames;
        lsw32(&p, (uint32_t)(int32_t)fr);
        const std::string &sp = g_sprites[i].path;
...
bool sep_unserialize_lua(const uint8_t *buf, size_t size)
{
    ...
    /* Read sprite list and rebuild g_sprites to match the saved IDs */
